...
def PSNR(a, b, max=255.0, shave_border=0):
    ...
    assert type(a) == paddle.Tensor or type(a) == np.ndarray
    ...
    if type(a) == paddle.Tensor:
        return 10.0 * (max ** 2 / ((a - b) ** 2).mean()).log10()
    elif type(a) == np.ndarray:
        return 10.0 * np.log10(max ** 2 / np.mean((a - b) ** 2))
    else:
        raise Exception('The PSNR function supports paddle.Tensor or np.ndarray types.', type(a))


def load_weights(model, path):
    state_dict = model.state_dict()
    for n, p in paddle.load(path).items():
